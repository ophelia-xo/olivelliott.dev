...
- [x] **PIX-01**: `/projects` page lists all projects with a filterable tag chip row
...
- [x] **RES-01**: Resume content is stored in a typed source-of-truth file (TypeScript or MDX) — not duplicated
- [x] **RES-02**: `/resume` route renders the resume as an HTML page from the source-of-truth
- [x] **RES-03**: `/resume` route has dedicated print CSS (`@media print`) so the same React tree prints correctly
- [x] **RES-04**: A build-step using Puppeteer (or equivalent) produces `/public/resume.pdf` from the `/resume` page
- [x] **RES-05**: The resume PDF is linked as a visible download on `/resume` and in the footer
- [x] **RES-06**: Resume content reflects current work, current role, real skills — no stale or fabricated entries
...
| PIX-01 | Phase 4 — Home + Projects Index | Complete |
