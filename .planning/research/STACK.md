...
**Domain:** High-touch, dark-theme developer portfolio (Next.js App Router, Vercel-hosted)
**Researched:** 2026-04-18
**Confidence:** HIGH (all core picks verified against current 2026 docs/releases; a few auxiliary picks MEDIUM where the decision is taste-driven)

---

## Executive Recommendation

For a greenfield 2026 portfolio that positions Olive as an engineer who cares about craft, the shortest path to a high-touch, dark-minimal, MDX-driven site is:

**Next.js 16.2 (App Router, RSC, Turbopack) + Tailwind CSS v4.1 + Motion 12 + Geist (via `next/font`) + @next/mdx with Shiki + shadcn/ui primitives (sparingly) + Lucide icons + Vercel Analytics + Biome.**

Content lives in-repo as MDX under `/content/projects/*.mdx` with typed frontmatter validated by Zod at build time — no Contentlayer, no CMS. 3D is deferred; custom SVG + CSS + Motion covers the "surprising-but-restrained" detail brief without a WebGL budget.

Everything below is picked because it (1) fits a dark-only, minimalist aesthetic, (2) avoids the AI-template look, (3) keeps Lighthouse ≥ 90 reachable, and (4) doesn't lock Olive out of adding interactive demos later.
...
pnpm create next-app@latest portfolio \
  --typescript --tailwind --eslint --app --use-pnpm --src-dir

cd portfolio

# Core runtime
pnpm add motion next-themes geist \
  clsx tailwind-merge zod \
  @vercel/analytics @vercel/speed-insights \
  lucide-react react-wrap-balancer

# MDX pipeline
pnpm add @next/mdx @mdx-js/react @mdx-js/loader \
  gray-matter \
...
# Dev tooling
pnpm add -D @biomejs/biome typescript @types/node @types/react @types/react-dom \
  @types/mdx lint-staged simple-git-hooks
```
...
**If a project case study needs an interactive demo later (e.g., Fathom cost calculator):**
...
**If Olive decides to add `/writing` post-v1:**
...
**If a light-mode toggle is ever added:**
...
**If custom domain ships and real traffic arrives:**
...
---
*Stack research for: high-touch, dark-theme Next.js App Router portfolio (olivelliott.dev)*
*Researched: 2026-04-18*
